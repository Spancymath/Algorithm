...
/**
 * 算法分析
 * 1.再多的实验也不一定能够证明我是对的，但只需要一个实验就能证明我是错的。
 * 2.算法分析中常用的近似函数
 *   调和级数求和 1+1/2+1/3+1/4+···+1/N ~ lnN
 *   等差数列求和 1+2+3+4+···+N ~ N^2 / 2
 *   等比数列求和 1+2+4+8+···+N = 2N-1 ~ 2N(N=2^n)
 ...
 *   指数级别 2^N
 * 4.2-sum:排序，找负数是否存在
 * 5.倍率定理：如果T(N) ~ aN^blgN, 那么T(2N)/T(N) ~ 2^b
 * 6.内存
 *   char 2字节
 *   int 4字节
 *   double 8字节
 *   long 8字节
 *   boolean 1字节
 * 7.一般内存的使用都会被填充位8字节（64位计算机中的机器字）的倍数
 */
